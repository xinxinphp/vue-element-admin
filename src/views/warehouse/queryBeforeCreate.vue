<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button :loading="createdownloadLoading" type="primary" icon="el-icon-download" @click="warehouseDaily('create')">生成报表</el-button>
      <el-button :loading="deletedownloadLoading" type="primary" icon="el-icon-delete" @click="warehouseDaily('delete')">删除报表</el-button>
    </div>
...
import { warehouseDailyQueryBeforeCreate, getWarehouseDailyCreate, getWarehouseDailyDelete } from '@/api/workshop'
...
export default {
  ...
  methods: {
    initFormSouch() {
      ...
      this.getList(warehouseDailyQueryBeforeCreate)
    },
    // 给 仓库 字段 进行赋值
    setWarehouse(data) {
    ...
    warehouseDaily(name) {
      const { factoryId, reportDate, warehouseId } = this.form
      if (!factoryId) {
        this.$message.error('必须选择一个工厂')
        return
      } else if (!reportDate) {
        this.$message.error('必须选择一个日期')
        return
      }
      this[name + 'downloadLoading'] = true
      const data = { factoryId, reportDate, warehouseId }
      const fn = { create: getWarehouseDailyCreate, delete: getWarehouseDailyDelete }
      fn[name](data)
        .then(res => {
          this[name + 'downloadLoading'] = false
